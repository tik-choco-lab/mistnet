...
namespace MistNet.DNVE3
{
    public class DNVE3Exchanger : IDisposable
    {
        ...
        private float[,] _selfDensitySnapshotBuffer;
        ...
        private SpatialDensityData _selfDensitySnapshotData;
        ...
        private void InitializeBuffers()
        {
            ...
            _selfDensitySnapshotBuffer = new float[dirCount, layerCount];
            ...
            _selfDensitySnapshotData = new SpatialDensityData
            {
                DensityMap = _selfDensitySnapshotBuffer,
                Position = new Position(Vector3.zero)
            };
        ...
        private async UniTask SendLoop(CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                ...
                Array.Copy(_selfDensityBuffer, _selfDensitySnapshotBuffer, _selfDensityBuffer.Length);
                _selfDensitySnapshotData.Position = _selfDensityData.Position;
                _dnveDataStore.SelfDensity = _selfDensitySnapshotData;
                ...
                foreach (var (_, info) in _dnveDataStore.Neighbors)
                {
                    ...
                    SpatialDensityUtils.MergeSpatialDensity(_mergedDensityBuffer, _selfDensityData.Position.ToVector3(), otherDensityMap, otherPos.ToVector3(), _mergedDensityBuffer);
                }
        ...
        private void UpdateSpatialHistogramData(List<Node> nodes)
        {
            ...
            SpatialDensityUtils.CreateSpatialDensity(selfPos, _posBuffer, nodes.Count, _selfDensityBuffer);
